...
EXT bool __stdcall GetRangeValData(double &upRangeVal, double &downRangeVal)
{
   ...
   //GetModuleFileNameW(moduleInstance,path,_countof(path));
   //MessageBox(0,path,L"INFO",MB_OK);
...
EXT bool __stdcall ModuleDestroy()
{
   ...
   /* BOOL res = 1;
      if(mainWindowHandle!=NULL)
       res = DestroyWindow(mainWindowHandle);
      if(res!=0)
       return(true);
      else
       return(false);*/
   ...
   //return((int)toolBarHWnd);
   //chartModule.StepForwardBtnClicked();

   //if(toolBarHWnd!=NULL)
   //   //SendDlgItemMessage(toolBarHWnd,IDC_STEP_FORWRD_BTN,WM_COMMAND,IDC_STEP_FORWRD_BTN,0);
   //   PostMessageA(toolBarHWnd,WM_COMMAND,IDC_STEP_FORWRD_BTN,0);
   //return(0);
}
