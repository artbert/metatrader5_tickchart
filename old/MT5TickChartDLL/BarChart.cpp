...
void CBarChart::UpdateChart(bool vScaleParChanged)
{
   if(initialized)
   {
      ...
      //if(barChartBarWidth>1)
      //{
      //   if(barChartBarWidth==2)
      //      extremumStartIndex=extremumStartIndex+extremumCount;
      //   else
      //      extremumStartIndex=extremumStartIndex+3*extremumCount;
      //}
      //static int ctr=0;
      ...
      if(maximum!= lastChartPriceMax || minimum!= lastChartPriceMin)
      {
         ...
         //if(mProfileDataVis)
         //{
         //mProfileStartIndex=((int)NormalizeDouble(minVal/_Point,_Digits)%100000);
         //int endInd=((int)NormalizeDouble(maxEvenVal/_Point,_Digits)%100000);
         ...
         //ctr++;
         //}
         //m_descriptors.Update(0,DoubleToString(((mxVal-mnVal)/_Point)/pipsDivider,doubleSignificantPlaces)+" pips, "+IntegerToString(barChartTickSize)+" tick");
         //m_descriptors.Update(0,IntegerToString(barChartTickSize)+" tick");
      }
      ...
      //char text[1000];
      //_i64toa_s(mProfileSize,text,1000,10);
...
void CBarChart::DrawData(const uint index)
{
   ...
   //int extremumCount=chartWidthInSamples;
   //int extremumStartIndex=seriesPointer-chartWidthInSamples+1;
   ...
   /* rct.left=x;
      rct.top=2;
      DrawTextOut_A(&rct,DESCRIPTION,DT_SINGLELINE | DT_NOCLIP|DT_LEFT|DT_TOP);*/

   if(vScaleParamsChanged)
      //DrawBitText_12(DESCRIPTION,10,2,2,false);
   ...
   for(; i<=seriesPointer; i++,x+=dx)
   {
      ...
      if(i>0)
      {
         if(interval>0)
         {
            if(interval==60)
            {
               ...
               if(hour1!=_hr)
               {
                  //LineVertical(x+1,10,m_data_area.bottom,timeIntervalObjColor);
            ...
            else
            {
               ...
               //ulong actualHourFragment=(ulong)(t1/intervalDivider);
               //ulong previousHourFragment=(ulong)(t2/intervalDivider);


               /* if(!once)
                  {
                  char text[1000];
                  _i64toa_s(actualHourFragment,text,1000,10);
                  DrawBitText_12(text,15,m_data_area.left+6,100,false);
                  once=true;
                  }*/

               //int actualHourFragment=(int)((times[i]%3600)/intervalDivider);
               //int previousHourFragment=(int)((times[i-1]%3600)/intervalDivider);
               ...
               if(actualHourFragment!=previousHourFragment)
               {
                  ...
                  //int hourMinSec=((times[i]/intervalDivider)*intervalDivider)%86400;;
                  //int hr = hourMinSec/3600;
                  //ulong t1=times[i];
                  //ulong days =(((t1 * 1158050442) >> 32) + t1 * 49710) >> 32;
                  //t1 -= days * 24*60*60;
                  //ulong hr1 = (t1 * 1193047) >> 32;
                  //t1 -= hr1 * 3600;
                  //ulong mn=((t1 * interval_factor) >> 32)*interval;
      ...
      if(mainPlotVis)
      {
         if(barChartBarWidth==4)
         {
            ...
            if(direction)
            {
               //openPr>=closePr : axis coordinates
               //LineVertical(x+1,highPr,lowPr,bearCandleColor);
               //LineVertical(x+1,closePr,openPr,bullCandleColor);

               //Rectangle(x,closePr,x+2,openPr,bearCandleColor);
            ...
            {
               //LineVertical(x+1,highPr,lowPr,bearCandleColor);
               //FillRectangle(x,closePr,x+2,openPr,bearCandleColor);
               SafeSortedLineVertical(x+1,highPr,lowPr,bearCandleColor);
         ...
         else
         {
            ...
            int highPr=(int)(m_y_0-high*m_scale_y+0.5);
            int lowPr=(int)(m_y_0-low*m_scale_y+0.5);
            //LineVertical(x,highPr,lowPr,bearCandleColor);
            //highPr is <= lowPr as it's Y axis coordinate
...
void CBarChart::DrawSignedLevels()
{
   ...
   //TextColorSet(signedLevelsTextColor);
   double vvvOffst=0;
   if(m_v_scale_min>0)
   ...
   for(int j=0; j<signedLevelsArraySize; j++)
   {
      ...
      {
         rct.top=y_val-11;
         //DrawTextOut_A(&rct,signedLevelsDescriptions[j],DT_SINGLELINE | DT_NOCLIP|DT_LEFT|DT_BOTTOM);
      ...
      if(multiplierValid)
      {
         if(dy==0)
         {
            ...
            for(int i=m_y_max; i<rangeHeight; i++)
            {
               if(rescaledMProfile[i]>0)
               {
                  ...
                  //Line(m_data_area.left+1,i,new_x,i,mProfileColor);
         ...
         else
         {
            ...
            if(mProfileData[mProfileStartIndex]>0)
            {
               ...
               //FillRectangle(m_data_area.left+1,yyy2,new_x,yyy2+1-dy,mProfileColor);
            ...
            for(int i=mProfileStartIndex+1; i<mProfileSize+mProfileStartIndex; i++)
            {
               ...
               if(mProfileData[i]>0)
               {
                  ...
                  //if(fillRect)
                  //   FillRectangle(m_data_area.left+1,finalY1,new_x,finalY2,mProfileColor);
                  //else
                  //{
                  //   Line(m_data_area.left+1,finalY1,new_x,finalY1,mProfileColor);
                  //}
               }
            }
         }
