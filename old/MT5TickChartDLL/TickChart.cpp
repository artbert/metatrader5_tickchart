...
void CTickChart::SetColorMode(const bool value)
{
   ...
   if(DarkMode)
   {
      ...
      //askLineColor=(uint)XRGB_gdi(255,255,255)-(uint)XRGB_gdi(220,0,0);
      //bidLineColor=(uint)XRGB_gdi(255,255,255)-(uint)XRGB_gdi(0,0,255);
      ...
      //timeParameterObjColorNeutral=(uint)XRGB_gdi(255,255,255)-(uint)XRGB_gdi(62,225,62);
...
void CTickChart::DrawBackground(void)
{
   ...
   else
   {
      ...
      ////Only temp
      //offst=&m_pixels[m_data_area.left+1];
      //offst+=100*m_width;
      //memset(offst,m_color_background,size);
      //offst+=m_width;
      //memset(offst,m_color_background,size);
      ...
      for(int i=1; i<m_width-1; i++)
      {
         ...
         {
            int index=m_width+i;//1*m_width + i
...
void CTickChart::FillSeries(const double askPrice,const double bidPrice,const time_t time,const uint timeParameter,const int cumulativeDataAsk,const int cumulativeDataBid)
{
   ...
   /* for(int i=0; i<totalMProfileSize; i++)
      {
      mProfileDataBid[i]=0;
      mProfileDataAsk[i]=0;
      }*/
...
void CTickChart::AppendPricesTimeAndParameters(const double askPrice,const double bidPrice,const time_t time,const ulong timeParameter,const int askChange,const int bidChange,const short road,const short distance,const short ticks)
{
   ...
   {
      //lastVal = mCumulativeCountDataBid[seriesPointer-1]
      //(if bidChange>0 && lastVal>=0)|| (bidChange<0 && lastVal<0)
...
void CTickChart::DrawChart(void)
{
   if(initialized)
   {
      ...
      //char text[100];
      //_itoa(interval,text,10);
      //RECT rct = {0};
      //rct.top=100;
      //rct.left=100;
      //DrawTextOut_A(&rct,text,DT_SINGLELINE | DT_NOCLIP|DT_LEFT|DT_TOP);
      ////SetDlgItemInt(hWnd, IDC_MAIN_EDT, interval, FALSE);
   }
}
void CTickChart::DrawData(const uint index)
{
   mDataAreaStartPoint=m_data_area.left;
   //tickSampleWidth==1|2|4
   //if tickSampleWidth==4 then tickSampleWidth>>2==1 else tickSampleWidth>>2==0
   ...
   //Przy za³o¿eniu, ¿e bid nie bêdzie wiêkszy od ask
   //Uwaga: wspó³rzêdne wykresu na osi Y rosn¹ w dó³.

   y2_ask=max(y2_ask,minY);
   y2_ask=min(y2_ask,maxY);
   y2_bid=max(y2_bid,minY);
   y2_bid=min(y2_bid,maxY);
   ...
   for(; i<=seriesPointer; i+=idxIncr,x+=dx)
   {
      if(tickSampleWidth==1)
      {
         ...
         y1_ask=max(y1_ask,minY);
         y1_ask=min(y1_ask,maxY);
         if(minPixelRealPrice>y1_ask)
            minPixelRealPrice=y1_ask;

         y1_bid=max(y1_bid,minY);
         y1_bid=min(y1_bid,maxY);
      ...
      else
      {
         ...
         y2_ask=(int)(m_y_0-val_ask*m_scale_y+0.5);//SPRAWDZIÆ, CZY DA SIÊ UPROŒCIÆ
         y2_bid=(int)(m_y_0-val_bid*m_scale_y+0.5);
      ...
      if(isCalendarEvents & showCalendarEvents)
      {
         //if(calEvStartInd>-1 && calEvEndInd>-1)
         //Obie liczby nieujemne - jedna instrukcja skoku warunkowego zamiast dwóch w przypadku u¿ycia operatora '&&'
         if(((calEvStartInd&0x80000000) | (calEvEndInd&0x80000000))==0)
         {
            ...
            while(calEvStartInd<=calEvEndInd)
            {
               if(times[i]>=calendarEvents[calEvStartInd].eventDateTime)
               {
                  ...
                  int ptr=0;
                  while(calendarEvents[calEvStartInd].eventSymbol[ptr]!=NULL)
                  {
                  ...
                  ptr=0;
                  while(calendarEvents[calEvStartInd].eventDescription[ptr]!=NULL)
                  {
                  ...
                  //rct.right=x-9;
                  //rct.top=calendarSignOffset+5-4;
                  //DrawTextOut_A(&rct,INFOSTRING,DT_SINGLELINE | DT_NOCLIP|DT_RIGHT|DT_VCENTER);
      ...
      if(isOrdersPoints)
      {
         //if(transactionPStartInd>-1 && transactionPEndInd>-1)
         //Jeœli logiczna suma bitów znaków == 0 :obie liczby s¹ nieujemne
      ...
      if(interval>0)
      {
         ...
         //   --prevTickIdx;//times[i-2];
         //   time_t t1=times[i];
         //   ulong hr1 = (t1 * 1193047) >> 32;
         //   t1 -= hr1 * 3600;
         //   ulong mn1 = t1 * 71582789 >> 32;

         //   time_t t2=times[i-2];
         //   ulong hr2 = (t2 * 1193047) >> 32;
         //   t2 -= hr2 * 3600;
         //   ulong mn2 = t2 * 71582789 >> 32;
         //   //Jeœli minuta aktualnego ticku ró¿ni siê od minuty poprzedniego ticku
         //   if(mn1!=mn2)
         //   {
         //      ulong a1 = (mn1*interval_factor)>>32;
         //      if((mn1-a1*interval)==0)
         //      {
         //         //LineVertical(x,10,m_data_area.bottom,timeIntervalObjColor);
         //         SafeSortedLineVertical(x,10,m_data_area.bottom,timeIntervalObjColor);
         //         erase_bkg_ver[x]=1;

         //         t1=times[i];
         //         ulong days =(((t1 * 1158050442) >> 32) + t1 * 49710) >> 32;
         //         t1 -= days * 24*60*60;
         //         hr1 = (t1 * 1193047) >> 32;

         //         INFOSTRING[0]=TIMETABLE_A[hr1][0];
         //         INFOSTRING[1]=TIMETABLE_A[hr1][1];
         //         INFOSTRING[2]=':';
         //         INFOSTRING[3]=TIMETABLE_A[mn1][0];
         //         INFOSTRING[4]=TIMETABLE_A[mn1][1];
         //         INFOSTRING[5]=NULL;

         //         DrawBitTimeSepStamp_09(INFOSTRING,5,x-11,2,m_data_area.left+1,m_data_area.right-1);
         //      }
         //   }
         //}
         //else
         ...
         if(mn1!=mn2)
         {
            ...
            if((mn1-a1*interval)==0)
            {
               //LineVertical(x+dx,10,m_data_area.bottom,timeIntervalObjColor);
      ...
      if(drawVerticalGrid)
      {
         if(((tickSampleWidth==1)&&(counter%40==0))
               || ((tickSampleWidth==2)&&(counter%20==0))
               || ((tickSampleWidth==4)&&(counter%10==0)))
      ...
      if(mTimeParameterVis)
      {
         ...
         if(cond)
         {
             ...
             //unsigned: subtracting bigger from smaller = wrap around (BIG NUMBER as result)
             if (_tmp_value > 255)
                 _tmp_value = 255;
             
             ulong tmParamIdx = 255 - _tmp_value;

             ////for little endian: value shifted 32 bits left will be saved in second pixel of m_pixels array if tickSampleWidth == 4
             //otherwize will be replaced by different value in the next iteration (next bar rate)
             tmParamClr = timeParamColors[tmParamIdx][colorIdx];
             //tmParamPaddingClr pointing to tmParamClr if tickSampleWidth == 4 or to background color otherwise
            ...
            if(chartVisibility[0]==1)
            {
               if(mCumulativeCountDataAsk[i]!=0)
               {
                  ...
                  else
                  {
                     ...
                     //LineVertical(rectDt1,anchorPoint,(anchorPoint-figureHeight),cumulativeAskDataColor);
      ...
      if(mainPlotVis)
      {
         if(chartVisibility[0]==1)
         {
            ...
            else
            {
               ...
               if(y1_ask>y2_ask)//Patrzymy na wsp. piksela nie wartoœæ ceny (góra mniej, dó³ wiêcej)
               {
                  y1_ask+=y2_ask;
               ...
               if(lastAsk[0]>=0 && lastAsk[1]>=0)
               {
                  ...
                  if(currentAsk[1]<lastAsk[0])//Patrzymy na wsp. piksela nie wartoœæ ceny (góra mniej, dó³ wiêcej)
                     //LineVertical(x,lastAsk[0],currentAsk[0],askLineColor);
                     SafeSortedLineVertical(x,currentAsk[0],lastAsk[0],askLineColor);
                  else if(currentAsk[0]>lastAsk[1])
                     //LineVertical(x,currentAsk[1],lastAsk[1],askLineColor);
                     SafeSortedLineVertical(x,lastAsk[1],currentAsk[1],askLineColor);
                  else
                  {
                     if(y1_ask!=y2_ask)
                        //LineVertical(x,y1_ask,y2_ask,askLineColor);
                        SafeSortedLineVertical(x,y1_ask,y2_ask,askLineColor);
                     else if((unsigned)(y1_ask-minY-1)<valid_range)
                        //PixelSet(x,y1_ask,askLineColor);
         ...
         if(chartVisibility[1]==1)
         {
            ...
            {
               if(y1_bid>y2_bid)//Patrzymy na wsp. piksela nie wartoœæ ceny (góra mniej, dó³ wiêcej)
               {
                  y1_bid+=y2_bid;
   ...
   /* int hr = int((t1%86400)/3600);
      int mn = int((t1%3600)/60);
      int sec = int((t1%60));*/
...
void CTickChart::DrawSignedLevels(void)
{
   ...
   for(int j=0; j<signedLevelsArraySize; j++)
   {
      ...
      //LineHorizontalDott(m_data_area.left+1,m_data_area.right-1,y_val,signedLevelsObjColor);
   ...
   if(mProfileSize>1)
   {
      ...
      if(showAsk | showBid)
      {
         ...
         if(dy==0)
         {
            ...
            //double mostSignificantPricePart=int((bidPrices[dataStartIndex]*_DigitsMultiplier)/100000)*pow(10.0,5-_Digits);
            ...
            vvv-=vvvOffst;
            //int yInd=(int)NormalizeDouble(m_y_0-vvv*m_scale_y,0);
            int yInd=(int)(m_y_0-vvv*m_scale_y+0.5);

            //if(yInd>-1 && yInd<rangeHeight)
            if((uint)yInd<(uint)rangeHeight)
            {
               //if(mProfileDataAsk[mProfileStartIndex]>0 && mProfileDataAskVis && chartVisibility[0]==1)
               if(showAsk)
                  rescaledMProfileAsk[yInd]+=mProfileDataAsk[mProfileStartIndex];

               //if(mProfileDataBid[mProfileStartIndex]>0 && mProfileDataBidVis && chartVisibility[1]==1)
               if(showBid)
            ...
            for(int i=mProfileStartIndex+1; i<mProfileSize+mProfileStartIndex; i++)
            {
               ...
               //if(m_v_scale_min>0)
               vvv-=vvvOffst;
               //yInd=(int)NormalizeDouble(m_y_0-vvv*m_scale_y,0);
               yInd=(int)(m_y_0-vvv*m_scale_y+0.5);

               //if(yInd>-1 && yInd<rangeHeight)
               if((uint)yInd<(uint)rangeHeight)
               {
                  //if(mProfileDataAsk[i]>0 && mProfileDataAskVis && chartVisibility[0]==1)
                  if(showAsk)
                     rescaledMProfileAsk[yInd]+=mProfileDataAsk[i];

                  //if(mProfileDataBid[i]>0 && mProfileDataBidVis && chartVisibility[1]==1)
                  if(showBid)
            ...
            //double maxBid = MaxInArray(compoundMProfileBid,rangeHeight);
            //double maxAsk = MaxInArray(compoundMProfileAsk,rangeHeight);

            //if(maxBid>-DBL_MAX && maxAsk>-DBL_MAX)
            //{
            if(maxAsk>maxBid)
            {
               if(maxAsk!=0)
         ...
         else
         {
            ...
            //double maxBid = MaxInArray(mProfileDataBid,totalMProfileSize,mProfileStartIndex,mProfileSize);
            //double maxAsk = MaxInArray(mProfileDataAsk,totalMProfileSize,mProfileStartIndex,mProfileSize);
         ...
         if(multiplierValid)
         {
            ...
            else
            {
               ...
               if(showAsk && mProfileDataAsk[mProfileStartIndex]>0)
               {
                  ...
                  //FillRectangle(m_data_area.left+1,yyy2,new_x,yyy2+1-dy,mProfileAskColor);
               ...
               if(showBid && mProfileDataBid[mProfileStartIndex]>0)
               {
                  ...
                  //FillRectangle(m_data_area.left+1,yyy2,new_x,yyy2+1-dy,mProfileBidColor);
               ...
               for(int i=mProfileStartIndex+1; i<mProfileSize+mProfileStartIndex; i++)
               {
                  ...
                  if(showBid && mProfileDataBid[i]>0)
                  {
                     ...
                     //if(fillRect)
                     //   FillRectangle(m_data_area.left+1,finalY1,new_x,finalY2,mProfileBidColor);
                     //else
                     //{
                     //   Line(m_data_area.left+1,finalY1,new_x,finalY1,mProfileBidColor);
                     //}
                  }
               }
            }
