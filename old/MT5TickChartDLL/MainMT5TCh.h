...
//HWND floatingLabel = NULL;
TCHMODSET appSets;
MMRESULT timerHandle=NULL;
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow, HWND parentChart)
{
   ...
   //floatingLabel = CreateWindowEx(WS_EX_TRANSPARENT,L"STATIC", L"ST_U",
   //                               WS_CHILD,
   //                               1000, 100, 500, 30,
   //                               mainWindowHandle, (HMENU)(1234),
   //                               hInstance, NULL);
   //SetWindowText(floatingLabel, L"123.456:");
   //ShowWindow(floatingLabel, nCmdShow);
   //UpdateWindow(floatingLabel);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
   ...
   case WM_CREATE:
   {
      //toolBarHWnd = CreateDialog(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_TOOLBAR),hWnd, ToolDlgProc);
   ...
   case WM_COMMAND:
      ...
      break;
   case UPDATE_CHARTS:
      //MessageBoxA(0,"UPDATE_CHARTS","INFO",MB_OK);
      chartModule.UpdateCharts((byte)wParam,lParam!=0);
      break;
   case REWIND_CHARTS:
      //MessageBoxA(0,"REWIND_CHARTS","INFO",MB_OK);
      chartModule.StepBackwardBtnClicked(true,lParam!=0);
      break;
   case AUTO_SCROLL_UPDATE:
      //MessageBoxA(0,"AUTO_SCROLL_UPDATE","INFO",MB_OK);
   ...
   //{
   //   if((HWND)lParam==floatingLabel)
   //   {
   //      HDC hdcStatic = (HDC) wParam;
   //      SetBkMode(hdcStatic, TRANSPARENT);
   //      SetTextColor(hdcStatic, floatingLabelClr);
   //   }
   ...
   //    if (pDIS->hwndItem == tickChartHWnd)
   //    {
   //        //HBRUSH testBrush = CreateSolidBrush( RGB(0,0,0 ) );
   //        //HBRUSH testBrush = CreateSolidBrush( RandomRGB_gdi() );
   //        //RECT rct;
   //        //rct.left=0;
   //        //rct.right=1100;
   //        //rct.bottom=430;
   ...
   //        //WCHAR info[20];
   //        //_itow(pDIS->rcItem.right, info, 10);
   //        //SetDlgItemText(toolBarHWnd, IDC_MAIN_EDT, info);


   //        //FillRect(pDIS->hDC, &pDIS->rcItem, testBrush);
   //        //FillRect(pDIS->hDC, &rct, testBrush);
   //        //DeleteObject(testBrush);
   //        //SetDlgItemInt(toolBarHWnd,IDC_MAIN_EDT,(UINT)&(pDIS->rcItem.right),false);
   //        //canv.Update(pDIS->hDC,0,0,(int)(&pDIS->rcItem.right),(int)(&pDIS->rcItem.bottom-&pDIS->rcItem.top),(int)(&pDIS->rcItem.left),(int)(&pDIS->rcItem.top));



   //    }
   //    else if(pDIS->hwndItem == barChartHWnd)
   //    {
   //        //HBRUSH testBrush = CreateSolidBrush( RGB(20, 20,20 ) );
   //        //FillRect(pDIS->hDC, &pDIS->rcItem, testBrush);
   //        //DeleteObject(testBrush);
   //    }
   //}
...
INT_PTR CALLBACK ToolDlgProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
   ...
   /* case WM_DRAWITEM:
      {
      LPDRAWITEMSTRUCT pDIS = (LPDRAWITEMSTRUCT)lParam;
      if(pDIS->hwndItem == GetDlgItem(hwnd,IDC_TIMELABEL_FROM))
      {
         chartModule.UpdateFromTimeLabel();
      }
      else if(pDIS->hwndItem == GetDlgItem(hwnd,IDC_TIMELABEL_TO))
      {
         chartModule.UpdateToTimeLabel();
      }
      return TRUE;
      }*/
   default:
...
void ToolDlgFunctionsPanelProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
   ...
   case IDC_LD_FROM_SERVER_BTN:
      ...
      //PostMessage(MT5ParentChart, WM_LBUTTONUP, 0, -LOAD_DATA_FROM_SERVER);
      //{
      //   OPENFILENAME ofn;
      ...
      //   ofn.hwndOwner = hwnd;
      //   ofn.lpstrFilter = L"Data Files (*.dat)\0*.dat\0All Files (*.*)\0*.*\0";
      ...
      //   {
      //      //SetDlgItemTextA(hwnd,IDC_MAIN_EDT,ofn.lpstrFile);
      //      chartModule.ReadTickDataFromFile(ofn.lpstrFile);
      //   }
      //}
      break;
   ...
   case IDC_CHART_AUTO_SCRL_CHBX:
      ...
      {
         //SetDlgItemInt(toolBarHWnd,IDC_MAIN_EDT,1,false);
         appSets.chartAutoScroll=true;
      }
      else
      {
         //SetDlgItemInt(toolBarHWnd,IDC_MAIN_EDT,0,false);
   ...
   case IDC_STEP_FORWRD_BTN:
   {
      ...
      //HDC hDC = GetDC(tickChartHWnd);
      //canv.FillRectangle(x1,y1,x2,y2,RandomRGB());
      //canv.DrawTextOut(&rect,L"Text Out",DT_SINGLELINE | DT_NOCLIP|DT_LEFT);
      //canv.DrawTextOut(&rect,L"Text Out",DT_SINGLELINE | DT_NOCLIP|DT_LEFT);


      /* wchar_t descriptions[]=L"Jakis tekst#Text2#Some text#asdfg#MainLvl#Ostatni text";
         wchar_t * singleToken;
         std::vector<wchar_t*>signedLevelsDescriptions;
         singleToken = wcstok (descriptions,L"#");
         while (singleToken != NULL)
         {
          signedLevelsDescriptions.push_back(singleToken);
          singleToken = wcstok (NULL,L"#");
         }

         for(int i=0; i<signedLevelsDescriptions.size(); i++)
         {
          rect.left=rand()%1100;
          rect.top=rand()%430;
          canv.DrawTextOut(&rect,signedLevelsDescriptions[i],DT_SINGLELINE | DT_NOCLIP|DT_LEFT);
         }*/



      //canv.Update(hDC);
      //ReleaseDC(tickChartHWnd,hDC);
   }
   break;
   ...
   case IDC_MEASURE_CHBX:
      ...
      //SetDlgItemInt(toolBarHWnd,IDC_STEP_SIZE_EDT,(UINT)(floatingLabel),false);

      //if(appSets.measurementTool)
      //   ShowWindow(floatingLabel, SW_RESTORE);
      //else
      //   ShowWindow(floatingLabel, SW_HIDE);
      //UpdateWindow(floatingLabel);
      //UpdateWindow(tickChartHWnd);
      //chartModule.RefreshTickChartWindow();
   ...
   case IDC_LEVEL_ADD_BTN:
      ...
      //PostMessage(MT5ParentChart, WM_LBUTTONDOWN, 555, 666);
      break;
   case IDC_LEVELS_DEL_BTN:
      chartModule.DeleteSignedLevelsBtnClicked();
      //PostMessage(MT5ParentChart, WM_LBUTTONDOWN, 111, 222);
      break;
   }
}
bool MenuItemsActivationProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
   ...
   case IDM_TICK_CHART_ZOOM_4:
   {
      ...
      //CheckMenuItem(mainMenu,(UINT)(appSets.tickChartZoom/2+200),MF_CHECKED);
   break;
   ...
   case IDM_PRINT_SCREEN:
   {
       ...
       //chartModule.PrintScreenClicked();
   }
   break;
   case IDM_TIMER_100:
   case IDM_TIMER_200:
...
LRESULT CALLBACK TickChartWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
   ...
   //case WM_CREATE:
   //{
   //   //MessageBox(hWnd, L"Draw Item", L"Info",MB_OK);
   //   //SetDlgItemInt(toolBarHWnd,IDC_MAIN_EDT,123,false);
   //   /* RECT rect;
   //      GetWindowRect(hWnd, &rect);
   //      HDC hDC = GetDC(hWnd);
   //      canv.Create(hDC,rect.right-rect.left,rect.bottom-rect.top);
   //      canv.FillEllipse(100,100,50,70,0xFF123456);
   //      canv.Update(hDC);
   //      ReleaseDC(hWnd,hDC);*/
   //   //MessageBox(0,TEXT("CREATE"),0,0);
   ...
   case WM_MOUSELEAVE:
   {
      if(appSets.measurementTool)
      {
         ...
         //SetDlgItemInt(toolBarHWnd,IDC_MAIN_EDT,0,false);
         //MessageBox(0,TEXT("g_TrackingMouse = FALSE"),0,0);
      }

      /* POINT pt;
         GetCursorPos(&pt);
         ScreenToClient(hWnd,&pt);
         SetDlgItemInt(toolBarHWnd,IDC_MAIN_EDT,pt.x,false);*/
   }
   break;
   case WM_LBUTTONUP:
   //{
   //   //MessageBox(0,TEXT("MOUSE UP"),0,0);
   //   if(appSets.measurementTool)
   //   {
   //      POINT pt;
   //      GetCursorPos(&pt);
   ...
   //      startPt = pt.x;*/
   //   //SetDlgItemInt(toolBarHWnd,IDC_MAIN_EDT,pt.x,false);
   //   //MessageBox(0,TEXT("MOUSE DOWN"),0,0);
   //   if(appSets.measurementTool)
   //   {
   //      POINT pt;
   //      GetCursorPos(&pt);
   ...
   case WM_MOUSEMOVE:
   {
      ...
      if(appSets.measurementTool)
      {
         ...
         ScreenToClient(hWnd,&pt);
         chartModule.TickChartTTipChanged(pt.x,pt.y,GetAsyncKeyState(VK_LBUTTON),szTitle);
         //SetDlgItemInt(toolBarHWnd,IDC_MAIN_EDT,GetAsyncKeyState(VK_LBUTTON),false);
         //RECT rect;
         //GetWindowRect(floatingLabel, &rect);
         //MoveWindow(floatingLabel,pt.x,pt.y,500,30,true);
         ////SetWindowPos(floatingLabel,HWND_TOP,pt.x,pt.y,0,0,SWP_NOSIZE);
         ////UpdateWindow(floatingLabel);
   ...
   case WM_PAINT:
      ...
      //canv.Update(hdc);
      //canv.Update(hdc,(int)ps.rcPaint.left,(int)ps.rcPaint.top,(int)(ps.rcPaint.right-ps.rcPaint.left),(int)(ps.rcPaint.bottom-ps.rcPaint.top),(int)(ps.rcPaint.left),(int)(ps.rcPaint.top));
      /* WCHAR info[20];
         _itow(ps.rcPaint.right-ps.rcPaint.left, info, 10);
         SetDlgItemText(toolBarHWnd, IDC_MAIN_EDT, info);*/
      EndPaint(hWnd, &ps);
      break;
      //case WM_DESTROY:
...
LRESULT CALLBACK EditControlProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
   ...
   case WM_KEYUP:
   {
      ...
      case VK_RETURN:
      {
         //SetDlgItemTextA(toolBarHWnd,IDC_MAIN_EDT,"Focused");
...
//unsigned int RandomRGB_gdi()
//{
//   return(XRGB_gdi(rand()%255,rand()%255,rand()%255));
//}
void InitTickChartModuleStruct(HWND hwnd)
{
   ...
   //CheckMenuItem(hm,IDM_VIS_LEVELS,MF_CHECKED);
...
int MainModuleProcedure(HWND parentWindow, HINSTANCE hInst, int cmdShow)
{
   ...
   RECT rect;
   GetWindowRect(tickChartHWnd, &rect);
   //canv.Create(tchhDC,rect.right-rect.left,rect.bottom-rect.top);
   //canv.FillEllipse(100,100,50,70,0xFF123456);
   //canv.Update(tchhDC);
...
DWORD WINAPI MainModuleThreadFunction(LPVOID lpParam)
{
   ...
   //canv.Destroy();
   UnregisterClass(szWindowClass, NULL);
   //MessageBoxA(0,"closing","closing",MB_OK);
