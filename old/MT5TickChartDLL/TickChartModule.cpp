...
CTickChartModule::CTickChartModule(void)
{
   ...
   //timeLabelFromHWnd=NULL;
   //timeLabelToHWnd=NULL;

   //dialogBoxFont = CreateFont(-8, 0, 0, 0, 0, FALSE, 0, 0, 0, 0, 0, 0, 0, L"MS Sans Serif");
   //dialogBoxFont = CreateFont(-12, 0, 0, 0, 0, FALSE, 0, 0, 0, 0, 0, 0, 0, L"Consolas");
   ...
   TimeSepVLine = 0;
   //memset(TIMESTAMP_FROM,0,20);
   //memset(TIMESTAMP_TO,0,20);
}
...
CTickChartModule::~CTickChartModule(void)
{
   if(timesTabInMs!=NULL)
      delete[] timesTabInMs;
   /* if(milisecondsTab!=NULL)
      delete[] milisecondsTab;*/
   if(bidsTab!=NULL)
      delete[] bidsTab;
   if(asksTab!=NULL)
      delete[] asksTab;
...
bool CTickChartModule::Initialize(HWND terminalParent,HWND rootWindow,HWND tickChartHWnd,HWND barChartHWnd,HWND toolBoxHWnd)
{
   ...
   //milisecondsTab = new short[dataSize];
   ...
   //timeLabelFromHWnd = GetDlgItem(hWnd,IDC_TIMELABEL_FROM);
   //timeLabelToHWnd=GetDlgItem(hWnd,IDC_TIMELABEL_TO);
   ...
   //tickChart1.ShowTravelledDistance((*appSets).distanceVis,false);
   //tickChart1.ShowTravelledRoad((*appSets).roadVis,false);
   //tickChart1.ShowTicksArrived((*appSets).ticksArrivedVis,false);
   ...
   //tm timeinfo= {0};
   //localtime_s(&timeinfo,&last_tick.time);
   //char date[20];
   ...
   //SetDlgItemTextA(hWnd,IDC_MAIN_EDT,TOOLTIPLABELCONTENT);


   //SetDlgItemText(hWnd,IDC_DATELABEL,L"----.--.--");
   //SetDlgItemText(hWnd,IDC_TIMELABEL_TO,L"----.--.-- --:--:--");
   ...
   /* if(seriesIndex>-1)
      {
      lastBidValue=bidsTab[seriesIndex];
      lastAskValue=asksTab[seriesIndex];
      }*/

   readyToUse=true;
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"Initialized");
   return(true);
}
...
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"LoadFromServerBtnClicked");

   PostMessage(TerminalParentChartHWnd, WM_LBUTTONUP, 0, -LOAD_DATA_FROM_SERVER);

   //TO DO: SEND MESSAGE TO MT5:Load from server

}
void CTickChartModule::LoadFromServerPartialBtnClicked()
{
   //SetDlgItemText(hWnd,IDC_STEP_SIZE_EDT,L"PART");
   BOOL success;
...
void CTickChartModule::ChartAutoScrollChBxChanged()
{
   ...
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"ChartAutoScrollChBxChanged");
}
void CTickChartModule::StepForwardBtnClicked()
{
   ...
   //long long tm1 = GetMicrosecondCount();
   ...
   if(!(*appSets).chartAutoScroll)
   {
      ...
      for(; i<=seriesIndex; i++)
      {
         ...
         if(i>0)
         {
            //timeInSec=(double)((timesTabInMs[i]-timesTabInMs[i-1])/1000.0);
      ...
      //ObjectSetInteger(m_chart_id,"CurrentTimePointer",OBJPROP_TIME,timesTab[i-1]);
      //ObjectSetDouble(m_chart_id,"CurrentPricePointerLine",OBJPROP_PRICE,bidsTab[i-1]);

      if((*appSets).autoMovingRange && i>0)
      {
         /* double beforeLastBid=0,beforeLastAsk=0;
            if(lastEndIndex>-1)
            {
            if(lastEndIndex!=i-1)
            {
               beforeLastBid=bidsTab[lastEndIndex];
               beforeLastAsk=asksTab[lastEndIndex];
            }
            }
            if(bidsTab[i-1]!=beforeLastBid && asksTab[i-1]!=beforeLastAsk)
            {*/
         //double halfDistance=abs(upRangeLineValue-downRangeLineValue)/2.0;
      ...
      //tickChartTimeFrameIndexRight=i-1;

      //int firstBarTimeInd=0;
      //if((*appSets).tickChartZoom==1)
      //   firstBarTimeInd=(tickChartTimeFrameIndexRight-tickChart1.GetSeriesSize())>=0 ?(tickChartTimeFrameIndexRight-tickChart1.GetSeriesSize()) : 0;
      //else if((*appSets).tickChartZoom==2)
      //   firstBarTimeInd=(tickChartTimeFrameIndexRight-tickChart1.GetSeriesSize()/2)>=0 ?(tickChartTimeFrameIndexRight-tickChart1.GetSeriesSize()/2) : 0;
      //else
      //   firstBarTimeInd=(tickChartTimeFrameIndexRight-tickChart1.GetSeriesSize()/4)>=0 ?(tickChartTimeFrameIndexRight-tickChart1.GetSeriesSize()/4) : 0;



      //DisplayTimeStamp(timesTab[firstBarTimeInd],timeLabelFromHWnd,TIMESTAMP_FROM);
      //DisplayTimeStamp(timesTab[tickChartTimeFrameIndexRight],timeLabelToHWnd,TIMESTAMP_TO);
      ...
      //long long tm2 = GetMicrosecondCount();

      //static double totalSum=0,divider=1,result=0;

      //totalSum+=(double)(tm2-tm1);
      //result=totalSum/divider;
      //divider++;

      //long long r = static_cast<long long>(result);
      ...
      ////rct.top=100;
      ////rct.left=100;
      //SetDlgItemTextA(hWnd,IDC_MAIN_EDT,text);

      ////_i64toa_s(chartSearchIndex,text,1000,10);
      ////SetDlgItemTextA(hWnd,IDC_STEP_SIZE_EDT,text);
      ////TextColorSet(0xFFFFFF);
      ////DrawTextOut_A(&rct,text,DT_SINGLELINE | DT_NOCLIP|DT_LEFT|DT_TOP);
   }
}
...
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"MeasureChBxChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).measurementTool,false);
...
void CTickChartModule::TickChartTTipChanged(int posX,int posY,short LBUTTON_state,wchar_t *defaultRootTitle)
{
   if(seriesIndex>-1)
   {
      ...
      //if(posX>=startPoint && posX<(startPoint+1000) && posY>0 && posY<427)
      if((uint)(posX-startPoint)<1000 && (uint)(posY-1)<426)
         //if(posX>=startPoint && posX<(startPoint+1000) && (uint)(posY-1)<426)
      {
         ...
         if(!LBUTTON_state)
         {
            ...
            //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(rootWnd),false);
         ...
         else
         {
            ...
            //double divider=_Point/PipsDivider;
...
void CTickChartModule::BarChartTTipChanged(int posX,int posY,short LBUTTON_state,wchar_t *defaultRootTitle)
{
   if(seriesIndex>-1)
   {
      ...
      static bool defTitleChanged=false;
      int startPoint=(int)tickChart1.GetDataAreaStartPoint();
      //if(posX>=startPoint && posX<(startPoint+1000) && posY>0 && posY<299)
...
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"SetRangeBtnClicked");
   if(chartSearchIndex==-1)
      return;
   BOOL success;
   ...
   if(success)
   {
      if(newRange>1)
      {
         ...
         //TO DO: SEND MESSAGE TO MT5:upRangeLineValue,downRangeLineValue
         PostMessage(TerminalParentChartHWnd, WM_LBUTTONUP, 0, -SET_RANGE);
      }
   }
}
...
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"SetBarTickSizeBtnClicked");
...
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"MoveRangeUpBtnClicked");
   if(seriesIndex==-1)
      return;
   BOOL success;
...
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"MoveRangeDownBtnClicked");
   if(seriesIndex==-1)
      return;
   BOOL success;
   uint valueToMove = (uint)GetDlgItemInt(hWnd,IDC_MAIN_EDT,&success,false);
...
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"SignedLevelsEditChBxChanged");
   //TO DO: SEND MESSAGE TO MT5:Select TT levels
}
void CTickChartModule::AddSignedLevelBtnClicked()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"AddSignedLevelBtnClicked");
   //TO DO: SEND MESSAGE TO MT5:ADD TT level
   if(seriesIndex==-1)
      return;
...
void CTickChartModule::DeleteSignedLevelsBtnClicked()
{
   ...
   if(MessageBox(hWnd, L"Delete all '_TT' HLine objects?", L"Deleting HLine objects",MB_YESNOCANCEL|MB_ICONQUESTION)==IDYES)
   {
      ...
      //TO DO: SEND MESSAGE TO MT5:Delete all TT level
      //UpdateSignedLevelsLines(true);
      //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"Deleting all signed lvls...");
   }
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"DeleteSignedLevelsBtnClicked");
}
void CTickChartModule::TickChartZoomChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"TickChartZoomChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).tickChartZoom,false);
   ...
   //int firstBarTimeInd=0;
   //if((*appSets).tickChartZoom==1)
   //   firstBarTimeInd=(seriesIndex-tickChart1.GetSeriesSize())>=0 ?(seriesIndex-tickChart1.GetSeriesSize()) : 0;
   //else if((*appSets).tickChartZoom==2)
   //   firstBarTimeInd=(seriesIndex-tickChart1.GetSeriesSize()/2)>=0 ?(seriesIndex-tickChart1.GetSeriesSize()/2) : 0;
   //else
   //   firstBarTimeInd=(seriesIndex-tickChart1.GetSeriesSize()/4)>=0 ?(seriesIndex-tickChart1.GetSeriesSize()/4) : 0;

   //DisplayTimeStamp(timesTab[firstBarTimeInd],timeLabelFromHWnd,TIMESTAMP_FROM);
   //DisplayTimeStamp(timesTab[tickChartTimeFrameIndexRight],timeLabelToHWnd,TIMESTAMP_TO);

   //m_LblTimeRange1.Text(TimeToString(timesTab[firstBarTimeInd],TIME_DATE|TIME_MINUTES|TIME_SECONDS));
   //m_LblTimeRange2.Text(TimeToString(timesTab[tickChartTimeFrameIndexRight],TIME_DATE|TIME_MINUTES|TIME_SECONDS));
}
void CTickChartModule::BarChartCandleWidthChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"BarChartCandleWidthChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).barChartCandleWidth,false);
   barChart1.SetBarChartBarWidth((*appSets).barChartCandleWidth,seriesIndex>-1);
}
void CTickChartModule::TickChartTimeSepChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"TickChartTimeSepChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).tickChartTimeSep,false);
   tickChart1.SetInterval((*appSets).tickChartTimeSep,seriesIndex>-1);
   //tickChart1.UpdateChart();
}
void CTickChartModule::BarChartTimeSepChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"BarChartTimeSepChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).barChartTimeSep,false);

   barChart1.SetInterval((*appSets).barChartTimeSep,seriesIndex>-1);
}
void CTickChartModule::FreezeRanges()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"FreezeRanges");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).freezeRanges,false);
}
void CTickChartModule::ResetCharts()
{
   ...
   {
      ResetData();
      //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"Resetting data...");
   }
}
...
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"AskLineVisChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).askLineVis,false);
   tickChart1.ChartVisibility(0,(int)(*appSets).askLineVis,seriesIndex>-1);
}
void CTickChartModule::BidLineVisChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"BidLineVisChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).bidLineVis,false);
   tickChart1.ChartVisibility(1,(int)(*appSets).bidLineVis,seriesIndex>-1);
};
void CTickChartModule::SignedLevelsVisChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"SignedLevelsVisChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).signedLevelsVis,false);
...
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"AutoMovingRangeChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).autoMovingRange,false);


   //autoMoveRangeChecked=(*appSets).autoMovingRange;
}
void CTickChartModule::MProfileAskVisChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"MProfileAskVisChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).mProfileAskVis,false);
   if(_Digits<=5)
   {
...
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"MProfileBidVisChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).mProfileBidVis,false);
   if(_Digits<=5)
   {
...
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"TickChartVisChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).tickChartVis,false);
   tickChart1.ShowMainPlot((*appSets).tickChartVis,seriesIndex>-1);
}
void CTickChartModule::BarChartVisChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"BarChartVisChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(int)(*appSets).barChartVis,false);

   barChart1.ShowMainPlot((*appSets).barChartVis,seriesIndex>-1);
}
void CTickChartModule::TimeParameterVisChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"TimeParameterVisChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(int)(*appSets).timeParamVis,false);
...
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"CumulativeAskVisChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).cumulativeAskVis,false);
   tickChart1.ShowCumulativeDataAsk((*appSets).cumulativeAskVis,seriesIndex>-1);
}
void CTickChartModule::CumulativeBidVisChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"CumulativeBidVisChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).cumulativeBidVis,false);
   tickChart1.ShowCumulativeDataBid((*appSets).cumulativeBidVis,seriesIndex>-1);
}
void CTickChartModule::DistanceVisChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"DistanceVisChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).distanceVis,false);
   if(EnableSpeedStats)
      tickChart1.ShowTravelledDistance((*appSets).distanceVis,seriesIndex>-1);
}
void CTickChartModule::RoadVisChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"RoadVisChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).roadVis,false);
   if(EnableSpeedStats)
      tickChart1.ShowTravelledRoad((*appSets).roadVis,seriesIndex>-1);
}
void CTickChartModule::TicksArrivedVisChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"TicksArrivedVisChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).ticksArrivedVis,false);
   if(EnableSpeedStats)
      tickChart1.ShowTicksArrived((*appSets).ticksArrivedVis,seriesIndex>-1);
}
void CTickChartModule::EventsOnTickChartVisChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"EventsOnTickChartVisChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).eventsOnTickChartVis,false);

   //tickChart1.ShowCalendarEvents((*appSets).eventsOnTickChartVis,seriesIndex>-1);
}
void CTickChartModule::EventsOnBarChartVisChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"EventsOnBarChartVisChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).eventsOnBarChartVis,false);


   //barChart1.ShowCalendarEvents((*appSets).eventsOnTickChartVis,seriesIndex>-1);
}
void CTickChartModule::OrdersVisChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"OrdersVisChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).ordersVis,false);
...
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"ColorTimeParamChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).colorTimeParam,false);
...
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"ZoomTimeParamChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).zoomTimeParam,false);
...
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"SaveTicksClicked");
   HANDLE hFile;
   bool bSuccess = false;
   ...
   if(hFile != INVALID_HANDLE_VALUE)
   {
      ...
      DWORD dwWritten;
      //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)dwFileSize,false);
      //SetDlgItemTextA(hWnd,IDC_MAIN_EDT,strContent.c_str());
...
{
    bool bSuccess = false;
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"PrintScreenClicked");
    //HANDLE hFile;
    //bool bSuccess = false;
    ...
    //    {
    //        bSuccess = true;
    //        SetDlgItemTextA(hWnd, IDC_MAIN_EDT, "Image saved");

    //    }

    //    CloseHandle(hFile);
    //}
    HDC hDCScreen = ::GetDC(rootWnd);//First get the handle to the screen    
    int nBitPerPixel = GetDeviceCaps(hDCScreen, BITSPIXEL);//Get the number of bits per pixel
    int nWidthScreen = 1100;// GetDeviceCaps(hDCScreen, HORZRES);
    int nHeightScreen = 726;// GetDeviceCaps(hDCScreen, VERTRES);
    ...
    m_MyImage.Save(pszFileName, Gdiplus::ImageFormatPNG);
    m_MyImage.ReleaseDC();
    SetDlgItemTextA(hWnd, IDC_MAIN_EDT, "Image saved");
    return(bSuccess);
}
...
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"RealTempoChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).realTempo,false);
}
void CTickChartModule::TimerIntervalChanged()
{
   //SetDlgItemText(hWnd,IDC_MAIN_EDT,L"TimerIntervalChanged");
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)(*appSets).timerInterval,false);
   if((*appSets).replayModeOn)
   {
            ...
            if(ReadFile(hFile, pszFileText, dwFileSize, &dwRead, NULL))
            {
               ...
               if(dataSize>0)
               {
                  ...
                  for(int i=1; i<dataSize*4; i+=4,dataIterator++)
                  {
                     ...
                     //milisecondsTab[dataIterator]=(short)lngPtr[i+1];
                  ...
                  //SetDlgItemText(hWnd,IDC_TIMELABEL_TO,L"0000.00.00 00:00:00");

                  TimeSepVLine=timesTabInMs[0];

                  //isTickDataReady=true;
               }
            }
         }
      }
...
int CTickChartModule::OnTimer()
{
   ...
   if((*appSets).realTempo)
   {
      if(chartSearchIndex>-1)
      {
          //For more accurate result calculating time spent while executing
          //StepForwardBtnClicked() function, then subtracting it from next realTempoValue
          LARGE_INTEGER StartingTime, EndingTime, ElapsedMicroseconds;
          LARGE_INTEGER Frequency;
          QueryPerformanceFrequency(&Frequency);
          QueryPerformanceCounter(&StartingTime);
...
void CTickChartModule::AppendLevels(const double levels[],const int levelsSize,char (*descriptions)[100],const int descrSize, const bool update)
{
   ...
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)update,false);
   //SetDlgItemInt(hWnd,IDC_MAIN_EDT,(UINT)seriesIndex,true);
   if(update && seriesIndex>-1)
   {
      //MessageBoxA(0,"AppendLevels","INFO",MB_OK);
...
bool CTickChartModule::SetSymbolParameters(SYMBOLSETS &sets,char *date, int dateChars)
{
   ...
   //char test[1000];
   //_i64toa(sets.initTime,test,10);
   //////sprintf_s(test,"%.*f",_Digits, sets.initAsk);
   ...
   //tm timeinfo= {0};
   //localtime_s(&timeinfo,&sets.initTime);
   //char date[20];
   //strftime(date,sizeof(date),"%Y.%m.%d %H:%M:%S",&timeinfo);
   ////SetDlgItemTextA(hWnd,IDC_DATELABEL,date);
   //MessageBoxA(0,date,"TIMESTAMP_FROM",MB_OK);

   //memcpy(TIMESTAMP_TO,TIMESTAMP_FROM,20);
   //MessageBoxA(0,TIMESTAMP_TO,"TIMESTAMP_FROM",MB_OK);
   //MessageBoxA(0,TIMESTAMP_TO,"TIMESTAMP_TO",MB_OK);
...
{
   //MessageBox(0,L"Tick",L"Tick",MB_OK);
   //SetDlgItemInt(hWnd,IDC_STEP_SIZE_EDT,(UINT)sizeof(lastTick),false);
   //return(true);

   //char test[1000];
   //_itoa_s(sizeof(MqlTick),test,10);
   ////sprintf_s(test,"%.*f",_Digits, sets.initAsk);
   //MessageBoxA(0,test,"INFO",MB_OK);

   //char test[1000];
   //_itoa_s(sizeof(SYMBOLSETS),test,10);
   ////sprintf_s(test,"%.*f",_Digits, sets.initAsk);
   //MessageBoxA(0,test,"INFO",MB_OK);
   //return(true);
   ...
   if(readyToUse)
   {
      ...
      if((*appSets).chartAutoScroll)
      {
         ...
         //tickSampleWidth==2 || tickSampleWidth==4
         PostMessageA(rootWnd,AUTO_SCROLL_UPDATE,0,0);
      }
   }
   return(true);
