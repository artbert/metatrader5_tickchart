...
//d - integer
//#define NormalizeDouble(x,d) ((floor(((x)*_Pow_int(10.0,d))+0.5))/_Pow_int(10.0,d))
...
//extern WCHAR YEARSTAB[60][2];


template<class _cls> inline _cls MaxInArray(_cls arr[],int arraySize,int start=0,int count=-1)
{
...
template<class _cls> inline _cls MaxInArray2(_cls arr[],int start,int count)
{
   _cls result = arr[start];
...
template<class _cls> inline void MinMax(_cls arr[],int start,int count, _cls &minOut, _cls &maxOut)
{
   _cls mn = arr[start];
   _cls mx = arr[start];
   for(int i=start+1; i<start+count; i++)
...
template<class _cls> inline _cls MinInArray2(_cls arr[],int start,int count)
{
   _cls result = arr[start];
   for(int i=start+1; i<start+count; i++)
   {
...
template<class _cls> inline _cls MinInArray(_cls arr[],int arraySize,int start=0,int count=-1)
{
   _cls result = arr[start];
   int limit = count==-1?arraySize:start+count;
   for(int i=start+1; i<limit; i++)
...
//inline bool IntegerToString_A(char *charArray,int arrSize,long long value)
//{
//}
//
//inline bool IntegerToString_A(char *charArray,int arrSize,long long value, int str_len=0,char fill_symbol=' ')
//{
//
//
//}
//
//inline bool DoubleToString(char *charArray, int arrSize, int digits)
//{
//
//
//}
//inline double CharArrayToDouble(char *charArray, int arrSize, int digits)
//{
//   double result = 0;
//   int i=0;
//   for(;i<digits;i++)
...
//
//   }
//   while(charArray[idx]!=NULL && (charArray[idx]>='0' && charArray[idx]<='9'))
//   {
//
//   }
//
...
class Canvas {
 ...
 public:
   ...
   //bool              FontSet(LPCTSTR name,const int size,const uint angle=0, const uint weight=0, bool italic=false, bool underline=false, bool strikeout=false);
...
//void              TextOut(int x,int y,string text,const uint clr,uint alignment=0);
   //void              DrawTextOut(LPRECT lprc,LPCTSTR text,const uint clr,uint alignment=0);
   ...
   //void              DrawBitTextSafe_12(char arrayOfSigns[],int arrSize, int x, int y);
